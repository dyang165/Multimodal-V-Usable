...
class ViltEmbeddingsNoImg(nn.Module):
    """
    Construct the text and patch embeddings.
...
class ViltEmbeddingsNoText(nn.Module):
    """
    Construct the text and patch embeddings.
    ...
    def forward(
        ...
        if image_token_type_idx is None:
            image_token_type_idx = 1
        image_embeds = image_embeds + self.token_type_embeddings(
            torch.full_like(image_masks, image_token_type_idx, dtype=torch.long, device=image_masks.device)
        )
...
#            print(attention_scores.shape, attention_mask.shape)
            ...
            #plt.figure()
            #plt.imshow(attention_scores[1][0].cpu().detach().numpy())
            #plt.colorbar()
...
class ViltModel(ViltPreTrainedModel):
    ...
    def forward(
        ...
        mask_modality_interaction: Optional[bool] = None,
        mask_self_interaction: Optional[bool] = None 
        ...
        if mask_modality_interaction:
            text_dim = input_ids.shape[-1]
            image_dim = attention_mask.shape[-1] - text_dim
            cross_modal_mask = torch.block_diag(torch.ones(text_dim, text_dim), torch.ones(image_dim, image_dim)).repeat(attention_mask.shape[0],1,1).to(attention_mask.device)
            attention_mask = torch.logical_and(cross_modal_mask, attention_mask[:,None,:])
        elif mask_self_interaction:
            text_dim = input_ids.shape[-1]
            image_dim = attention_mask.shape[-1] - text_dim
            cross_modal_mask = torch.logical_not(torch.block_diag(torch.ones(text_dim, text_dim), torch.ones(image_dim, image_dim))).repeat(attention_mask.shape[0],1,1).to(attention_mask.device)
            attention_mask = torch.logical_and(cross_modal_mask, attention_mask[:,None,:])
...
@add_start_docstrings(
    "The ViLT Model transformer with no img outputting raw hidden-states without any specific head on top.",
    VILT_START_DOCSTRING,
)
class ViltModelNoImg(ViltPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = ViltEmbeddingsNoImg(config)
        self.encoder = ViltEncoder(config)
...
class ViltModelNoText(ViltPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = ViltEmbeddingsNoText(config)
        self.encoder = ViltEncoder(config)
...
class ViltEmbeddingsNoInput(nn.Module):
    """
    Construct the text and patch embeddings.
    ...
    def forward(
        ...
        embeddings = torch.ones([bs,1, hidden_dim]).to(device)
        masks = torch.ones([bs,1]).to(device)

        return embeddings, masks


class ViltModelNoInput(ViltPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = ViltEmbeddingsNoInput(config)
        self.encoder = ViltEncoder(config)
